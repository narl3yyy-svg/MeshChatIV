...
describe("mapLinkUtils security and XSS-related parsing", () => {
    ...
    it("findMapUriInContent stops before angle brackets and whitespace", () => {
        expect(findMapUriInContent("x meshchatx://map?lat=1&lon=2&z=3<evil")).toBe(
            "meshchatx://map?lat=1&lon=2&z=3",
        );
    ...
    it("classifies ping only from line-start prefix or label=ping", () => {
        ...
        expect(mapLinkKindFromMessage("MeshChatX map ping: " + uri, parseMeshchatMapUri(uri))).toBe(
            "ping",
        );
